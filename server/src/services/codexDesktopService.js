...
function buildModelDecodedRam(gameDataJson, exposure) {
  ...
  const recentText = Array.isArray(gameDataJson?.recent_visible_text)
    ? gameDataJson.recent_visible_text.filter((entry) => isValidatedRecentVisibleText(entry, gameDataJson)).slice(-6)
    : [];
  const fieldMoves = gameDataJson?.field_move_affordances && typeof gameDataJson.field_move_affordances === "object"
    ? gameDataJson.field_move_affordances
    : gameDataJson?.ram_assisted?.field_move_affordances || null;
  const currentConnections = gameDataJson?.current_connections && typeof gameDataJson.current_connections === "object"
    ? gameDataJson.current_connections
    : gameDataJson?.ram_assisted?.current_connections || null;
  const pathfindingAvailable = gameDataJson?.ram_assisted?.pathfinding?.available === true;
  const decoded = {
    screen_phase: screenPhase.phase || "inspect_screenshot",
    position: shouldExposeDecodedNavigation(exposure)
      ? modelVisiblePositionSurface(
          pos,
          coordinateFrame,
          shouldExposeDecodedField(exposure, "facing"),
          exposure.navigation?.validated === true || exposeAllDecodedRamForModel(exposure)
        )
      : null,
    movement_mode: shouldExposeDecodedField(exposure, "movement")
      ? {
          mode: gameDataJson?.player_movement_mode || movement.mode || "UNKNOWN",
          vehicle: movement.vehicle || null,
          surfing: movement.surfing === true,
    ...
    path_to_location_available: pathfindingAvailable && shouldExposeDecodedNavigation(exposure),
    collision_grid:
      pathfindingAvailable && shouldExposeDecodedField(exposure, "romCollision")
        ? sanitizeCollisionGridForModel(gameDataJson)
        : null,
    visibility: shouldExposeDecodedField(exposure, "visibility") ? sanitizeVisibilityForModel(gameDataJson) : null,
    field_move_affordances:
      shouldExposeDecodedField(exposure, "fieldMoveAffordances") && shouldExposeDecodedNavigation(exposure)
        ? sanitizeFieldMoveAffordancesForModel(fieldMoves)
        : null,
    current_connections:
      shouldExposeDecodedField(exposure, "currentConnections") && shouldExposeDecodedNavigation(exposure)
        ? sanitizeCurrentConnectionsForModel(currentConnections)
        : null,
    visible_warps:
      shouldExposeDecodedField(exposure, "warps") && shouldExposeDecodedNavigation(exposure)
        ? sanitizeVisibleWarpsForModel(gameDataJson?.visible_warps, gameDataJson)
        : null,
    visible_interactables:
      shouldExposeDecodedField(exposure, "interactables") && shouldExposeDecodedNavigation(exposure)
        ? (
            exposure.fields?.interactables?.validated === true && exposure.navigation?.validated === true
              ? sanitizeVisibleInteractablesForModel(gameDataJson, exposure)
    ...
    runtime_objects: shouldExposeDecodedField(exposure, "npcs")
      ? (
          validatedRuntimeObjectSurface(gameDataJson, exposure).validated === true
    ...
    progress: shouldExposeDecodedField(exposure, "progress")
      ? modelVisibleProgressFlagsSurface(gameDataJson?.progress_flags)
      : null,
  ...
  return sanitizeModelValue(pruneEmptyDecodedRam(decoded));
}

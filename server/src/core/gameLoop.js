...
const { callCodexCliForAction } = require('../services/codexCliService');
...
const { buildObservationExposure, validatedRuntimeObjectEntries } = require('../ai/observationContract');
...
const PERSISTENT_VALIDATED_CONFIDENCE = new Set([
    "verified",
    "verified_ram",
    "validated",
    "validated_ram",
...
function sanitizeProviderOutputsForHistory(output) {
    if (!Array.isArray(output)) return output;
    return output.map((item) => sanitizeModelValue(item));
}

function sanitizeSummaryEntry(entry) {
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
        return { text: sanitizeModelText(entry || ""), step: null, timestamp: null };
    }
    const safe = sanitizeModelValue(entry);
    return {
        ...safe,
        text: sanitizeModelText(safe.text || ""),
    };
}

function sanitizeSummaryEntries(entries) {
    return Array.isArray(entries) ? entries.map(sanitizeSummaryEntry) : [];
}

function persistentFieldValidated(gameDataJson, field) {
    const details = gameDataJson?.stateReliabilityDetails?.[field] || {};
    const values = [details.confidence, details.contract, details.source].map((value) =>
        String(value || "").trim().toLowerCase()
    );
    return values.some((value) => PERSISTENT_VALIDATED_CONFIDENCE.has(value));
}

function updatePersistentBadgeHistory(gameDataJson) {
    ...
    const observationExposure = buildObservationExposure(gameDataJson);
    const badgesValidated =
        !observationExposure.heartgold ||
        (observationExposure.fields?.badges?.validated === true && observationExposure.diagnosticsAllowed !== true);
    if (!badgesValidated) {
        console.warn(`Badge data unavailable from persistent badge history: ${observationExposure.fields?.badges?.confidence || "unknown"} confidence.`);
        return false;
    }
...
function reconcileMarkersWithNpcEntries(gameDataJson) {
    ...
    const npcsValidated =
        !observationExposure.heartgold ||
        (observationExposure.fields?.npcs?.validated === true &&
            observationExposure.navigation?.validated === true &&
            observationExposure.diagnosticsAllowed !== true);
    if (!npcsValidated) {
        return false;
    }
    const npcEntries = validatedRuntimeObjectEntries(gameDataJson);
    const currentMapId = gameDataJson?.current_trainer_data?.position?.map_id;
    if (!npcEntries.length || !currentMapId) return false;
...
async function gameLoop() {
    let stepsThisProcess = 0;
    while (true) {
        if (config.maxAgentSteps > 0 && stepsThisProcess >= config.maxAgentSteps) {
            console.log(`AGENT_MAX_STEPS=${config.maxAgentSteps} reached. Stopping game loop.`);
            setIsThinking(false);
            return;
        }
        ...
        try {
            ...
            const mapName = gameDataJson.current_trainer_data?.position?.map_name;
            const lastVisitedMapsUpdated = updateLastVisitedMaps(mapId, mapName, gameDataJson);
            ...
            updatePersistentBadgeHistory(gameDataJson);
            // --- End Badge Update Check ---

            // --- Check for Map First Visit ---
            const firstVisitExposure = buildObservationExposure(gameDataJson);
            const mapVisitValidated = !firstVisitExposure.heartgold || firstVisitExposure.navigation?.validated === true;
            const currentMapId = sanitizeModelText(String(gameDataJson.current_trainer_data?.position?.map_id ?? "")).slice(0, 120);
            const currentMapName = sanitizeModelText(String(gameDataJson.current_trainer_data?.position?.map_name || `Unknown Map (${currentMapId})`)).slice(0, 200);
            if (mapVisitValidated && currentMapId && !state.mapVisitHistory[currentMapId] && currentMapId != "0-0") {
                const visitInfo = {
                ...
                console.log(`>>> FIRST VISIT TO MAP: ${currentMapId} (${currentMapName}) at step ${state.counters.currentStep} <<< `);
            }
            ...
            const benchmarkMetrics = recordObservation(gameDataJson, {
                step: state.counters.currentStep,
            ...
            const screenshotCacheKey =
                ...
                null;
            const screenshotUrl = screenshotCacheKey
                ? `/screenshot/snapshot/${encodeURIComponent(String(screenshotCacheKey))}.png`
                : "/screenshot/raw";
            broadcast({
                type: 'full_state', // Or create specific update types
                payload: {
                    game: gameDataJson.game || null,
                    emulator: gameDataJson.emulator || null,
                    screenshot_raw_path: gameDataJson.screenshot_raw_path || gameDataJson.emulator?.screenshotRawPath || null,
                    screenshotSnapshotPath: gameDataJson.screenshotSnapshotPath || gameDataJson.emulator?.screenshotSnapshotPath || null,
                    screenshotCacheKey,
                    screenshotHash: gameDataJson.screenshotHash || gameDataJson.emulator?.screenshotHash || null,
                    screenshotAgeMs: gameDataJson.screenshotAgeMs ?? gameDataJson.emulator?.screenshotAgeMs ?? null,
                    screenshotFresh:
                    ...
                    pc_items: gameDataJson.pc_items,
                    pc_data: gameDataJson.pc_data,
                    objectives: state.objectives,
                    visible_area_data: gameDataJson.visible_area_data,
                    open_dialog_text: gameDataJson.open_dialog_text,
                    important_events: gameDataJson.important_events,
                    npc_entries: gameDataJson.npc_entries,
                    npc_entries_visible: gameDataJson.npc_entries_visible,
                    is_talking_to_npc: gameDataJson.is_talking_to_npc,
                    is_in_battle: gameDataJson.is_in_battle,
                    battle_data: gameDataJson.battle_data,
                    ...
                    progressSteps: state.progressSteps,
                    benchmarkMetrics,
                    stuckState: benchmarkMetrics.stuck,
                    ...
                    strength_enabled: gameDataJson.strength_enabled,
                    player_movement_mode: gameDataJson.player_movement_mode,
                    benchmark_note: gameDataJson.benchmark_note,
                    stateReliability: gameDataJson.stateReliability || gameDataJson.game?.stateReliability || null,
                    observationFreshness: gameDataJson.observationFreshness || null,
                    stateReliabilityDetails: gameDataJson.stateReliabilityDetails || null,
                }
            });
            ...
            const {
                ...
                image2Base64,
                error: visionError
            } = await buildVisionPayload(gameDataJson);
            if (!image1Base64) {
                console.error(`Could not build vision payload: ${visionError || "Unknown error"}. Pausing...`);
                recordHarnessFailure("vision_payload_unavailable", visionError || "Could not build vision payload", {
                    screenshotHash: gameDataJson?.screenshotHash || gameDataJson?.observationFreshness?.screenshotHash || null,
                    screenshotAgeMs: gameDataJson?.screenshotAgeMs || gameDataJson?.observationFreshness?.screenshotAgeMs || null,
                    observationMode: gameDataJson?.game?.observationMode || config.observation.mode,
                });
                await new Promise(resolve => setTimeout(resolve, 5000)); // Wait 5s
                continue;
            }
            ...
            // The Codex CLI provider is intentionally API-free. Keep the loop on
            // gameplay actions instead of falling through to the Responses API
            // summary path until a CLI summary adapter is enabled.
            const shouldSummarize = config.agentProvider === "codex-cli"
                ? false
                : (shouldSummarizeBasedOnSteps || shouldSummarizeBasedOnTokens); // <<< Updated condition
            ...
            if (shouldSummarize) {
                        ...
                        switch (event.type) {
                            ...
                            case "response.completed":
                                ...
                                const summaryDuration = Date.now() - summaryStart;
	                                ...
	                                newSummaryText = sanitizeModelText(summaryItem?.content?.find(item => item.type === "output_text")?.text || "[Summary Error]");
                                    if (finalResponse?.output) {
                                        finalResponse.output = sanitizeProviderOutputsForHistory(finalResponse.output);
                                    }
                ...
                const existingAllSummaries = sanitizeSummaryEntries(state.allSummaries);
                const existingSummaries = sanitizeSummaryEntries(state.summaries);
                ...
                if (nextSummaries.length >= 10) {
                    ...
                    try {
                        ...
                        const rollupMessage = Array.isArray(rollupFinalResponse.output)
                            ...
                            : null;
                        const rollupText = sanitizeModelText(
                ...
                if (finalResponse?.output) {
                    sanitizeProviderOutputsForHistory(finalResponse.output).forEach(item => {
            ...
            // 5. Call the configured model backend.
            console.log(`\n--- Sending to ${config.agentProvider === "codex-cli" ? "Codex CLI" : "OpenAI"} ---`);
            // console.log("API Input (history size):", apiInput.length); // Debug


            // Only criticize if enough steps have passed AND we are not summarizing this turn
            const shouldCriticize = config.agentProvider === "codex-cli"
                ? false
                        ...
                        switch (event.type) {
                            ...
                            case "response.completed":
                                ...
                                const criticismDuration = Date.now() - criticismStart;
	                                ...
	                                const criticismText = sanitizeModelText(criticismItem?.content?.find(item => item.type === "output_text")?.text || "[Criticism Error]");
                                    if (finalResponse?.output) {
                                        finalResponse.output = sanitizeProviderOutputsForHistory(finalResponse.output);
                                    }
            ...
            if (config.agentProvider === "codex-cli") {
                console.log("\n--- Sending to Codex CLI ---");
                broadcast({ type: 'reasoning_chunk', payload: `[codex-cli] Thinking with ${config.codexCli.model}...\n` });
                const codexResult = await callCodexCliForAction({
                    apiInput,
                    gameDataJson,
                    reasoningEffort,
                    imagePath: image1Path,
                    imageMeta: image1Path
                        ? {
                            width: image1Width,
                            height: image1Height,
                            rawWidth,
                ...
                finalResponse = codexResult.response;
                responseCompleted = true;
                const messageText = finalResponse.output
                    ?.find(item => item.type === "message")
                    ?.content
                    ?.find(item => item.type === "output_text")
                    ?.text || "";
                currentOutputText = messageText;
                broadcast({ type: 'reasoning_chunk', payload: `${messageText}\n` });
                broadcast({ type: 'reasoning_end', payload: null });
                recordReasoning({
                    type: "main",
                    model: `codex-cli:${config.codexCli.model}`,
                    serviceTier: "local-codex-cli",
                    durationMs: codexResult.meta.durationMs,
                });
                recordModelCall({
                    provider: "codex-cli",
                    model: config.codexCli.model,
                    reasoningEffort: codexResult.meta.reasoningEffort,
                    durationMs: codexResult.meta.durationMs,
                    imagePath: codexResult.meta.imagePath,
                    promptPath: codexResult.meta.promptPath,
                    outputPath: codexResult.meta.outputPath,
                });
                console.log(`Codex CLI completed in ${codexResult.meta.durationMs}ms. Output: ${codexResult.meta.outputPath}`);
            } else {
                const stream = await openai.responses.create({
                    model: config.openai.model,
            ...
            const modelSafeResponseOutput = finalResponse.output
                ? sanitizeProviderOutputsForHistory(finalResponse.output)
                : [];
            if (modelSafeResponseOutput.length > 0) {
                modelSafeResponseOutput.forEach(item => {
            ...
            if (finalResponse.output) {
                for (const item of finalResponse.output) {
                    if (item.type === "function_call") {
                        ...
                        state.history.push(sanitizeModelValue(toolResult)); // Add the model-safe tool result to the history
                    }
                }
            }
            if (!haveToolCall) {
            ...
            recordHarnessFailure("main_loop_error", error.message, { stack: error.stack });
...
module.exports = { gameLoop, updatePersistentBadgeHistory };
